...
noise_type = 'uniform_arc';        % 'gaussian' | 'uniform_time' | 'uniform_arc' | 'ar' | 'ma' | 'arma'
noise_std = 0.1;               % gaussian std [m]
noise_uniform_range = 0.1;     % uniform range [m]
% AR/MA/ARMA spatial noise (as a function of arc-length s)
ar_phi = 0.98;                 % AR(1) coefficient (|phi|<1)
ma_theta = 0.0;                % MA(1) coefficient
arma_phi = 0.98;               % ARMA(1,1) phi
arma_theta = 0.2;              % ARMA(1,1) theta
noise_std_eps = 0.02;          % [m] innovation std for AR/MA/ARMA
noise_smooth_win = 25;         % samples; 0/1 disables smoothing
...
if enable_noise
    ...
    elseif strcmp(noise_type,'ar')
        noise_params.phi = ar_phi;
        noise_params.std_eps = noise_std_eps;
        noise_params.smooth_win = noise_smooth_win;
    elseif strcmp(noise_type,'ma')
        noise_params.theta = ma_theta;
        noise_params.std_eps = noise_std_eps;
        noise_params.smooth_win = noise_smooth_win;
    elseif strcmp(noise_type,'arma')
        noise_params.phi = arma_phi;
        noise_params.theta = arma_theta;
        noise_params.std_eps = noise_std_eps;
        noise_params.smooth_win = noise_smooth_win;
    end
