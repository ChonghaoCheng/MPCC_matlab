function [u_opt, ds_opt, xpred] = solve_mpcc_3d(x0, s0, N, dt, r, r1, r2, ...
    Wc, Wl, Qs, Ru, u_last, v_lin_max, w_ang_max, v_progress_max, v_plan, lambda_du)
% SOLVE_MPCC_3D 
...
ds_guess = 0.5*v_progress_max * dt; 
...
% Control smoothing with separate weights for v/v_s/w
if nargin >= 17 && ~isempty(lambda_du) 
    Iu = eye(nu);
    D = kron(eye(N), Iu) - kron(diag(ones(N-1,1),-1), Iu);
    d0 = zeros(nu*N,1); d0(1:nu) = -u_last;
    % lambda_du can be scalar or [lambda_v, lambda_w, lambda_vs]
    if isscalar(lambda_du)
        ...
        lambda_vs = lambda_du(3);
    end
    W_smooth = kron(eye(N), diag([lambda_v, lambda_v, lambda_v, lambda_w, lambda_w, lambda_w, lambda_vs]));
    H = H + 2 * (D.' * W_smooth * D);
    f = f + 2 * (D.' * W_smooth * d0);
end
