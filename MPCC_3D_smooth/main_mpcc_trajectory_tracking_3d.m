...
%% Parameters 
traj_type = 'circle';   % 'line' | 'circle' | 'helix'
Tf        = 60.0;      % [s]
dt        = 0.01;      % [s]
N         = 10;        % horizon
...
lambda_du_mpc  = [1e-2, 1e-3];      % [linear_vel, angular_vel] for MPC
lambda_du_mpcc = [1e-2, 1e-3, 1e-3]; % [linear_vel, angular_vel, progress_speed] for MPCC
...
noise_type = 'uniform_time';        % 'gaussian' | 'uniform_time' | 'uniform_arc'
...
s0 = 0.0;
p0 = r(s0);
% Align initial orientation with path tangent at s0
...
v_plan_mpc = 2*S_end / Tf;
sdot_nom = v_plan_mpc;

% Get initial reference velocities to align with trajectory
% This reduces initial oscillations by starting at the correct velocity
s_ref_init = s0;
ds_ref_init = v_plan_mpc * dt;
[xref_init, uref_init] = sample_ref_path3d(s_ref_init, 1, ds_ref_init, r, r1, r2, v_plan_mpc);

% Set initial control inputs to match reference trajectory velocity
u0_mpc  = uref_init(:, 1);  % 6D initial control for MPC
u0_mpcc = [uref_init(:, 1); v_plan_mpc];  % 7D initial control for MPCC [vx,vy,vz,ωx,ωy,ωz,vs]
...
for k = 1:K
    ...
    if ~mpc_completed
        ...
        u_last_mpc = u_mpc; % update for smoothness
        % Check completion 
    ...
    % ==== MPCC ====
    if ~mpcc_completed
        % End-of-path gating: as we approach the end, remove progress reward
        % and cap progress speed to avoid overshoot and oscillation
        rem_s = max(0, S_end - s_mpcc);
        near_end = rem_s < 0.3; % [m] window
        Qs_local = Qs;
        v_progress_cap = v_progress_max;
        if near_end
            Qs_local = 0; % stop encouraging progress near the end
            % prevent overshoot: cap v_s so that s does not exceed S_end in one step
        ...
        u_last_mpcc = u_mpcc; % update for smoothness
    ...
    % Termination when both controllers are completed
    % or when both references have effectively reached the end in arc-length
    tol_s = 1e-3;
    if (mpc_completed || (s_ref_k >= S_end - tol_s)) && (mpcc_completed || (s_mpcc >= S_end - tol_s))
        break;
    end
end
